...
#model_name = "models/mobilenet_v1-1.0-imagenet-pruned.onnx"
model_name = args.model
input_dims, output_dims = get_input_output(model_name)
ov_model = ov.convert_model(model_name)
...
inputs = {_i: np.random.normal(0, 2.5, size=tuple(_dim)).astype(np.float32) for _i, _dim in enumerate(input_dims.values())} 

t3 = time.time()
while time.perf_counter() < stream_end_time:
    t1 = time.perf_counter()
    #output = compiled_model({0: inputs})
...
if args.iteration is None:
    filename = "{}_openvino_results.csv".format(model_title)
else:
    filename = "{}_openvino_results_{}.csv".format(model_title, args.iteration)
